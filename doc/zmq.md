...
The Lorene Core daemon can be configured to act as a trusted "border
router", implementing the lorene wire protocol and relay, making
...
The ZeroMQ feature in Lorene Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building lorened:
...
For instance:
    ...
    $ lorened -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/lorened.tx.raw \
...
These options can also be provided in lorene.conf.
...
From the perspective of lorened, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into lorened directly. Furthermore, no information is
...
using. Lorened appends an up-counting sequence number to each
